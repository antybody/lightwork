...
var miao=values.get("1");
if(miao.indexOf('/')>0){
	miao=miao.replace('/','秒钟后开始,间隔')+"秒钟";
}else{
	
}
var fen=values.get("2");
if(fen.indexOf('/')>0){
	fen=fen.replace('/','分钟后开始,间隔')+"分钟";
}else{
	
}
var shi=values.get("5");
var day=values.get("6");
var month=values.get("7");
var week=values.get("8");
var cronstr='';
if(miao.indexOf('*')!=0&&miao!='0'){
	cronstr+="秒:"+miao+";";
}
if(fen.indexOf('*')!=0&&fen!='0'){
	cronstr+="分:"+fen+";";
}
if(shi.indexOf('*')!=0&&shi.indexOf('?')!=0){
	cronstr+="时:"+shi+";";
}
if(day.indexOf('*')!=0&&day.indexOf('?')!=0){
	cronstr+="天:"+day+";";
}
if(month.indexOf('*')!=0&&month.indexOf('?')!=0){
	cronstr+="月:"+month+";";
}
if(week.indexOf('*')!=0&&week.indexOf('?')!=0){
	cronstr+="周:"+week+";";
}
var fen2=values.get("2");
if(values.get("1")!=0){
	fen2="*";
}
var cron=values.get("1")+" "+fen2+" "+values.get("5")+" "+values.get("6")+" "+values.get("7")+" "+values.get("8")
