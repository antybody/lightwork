...
//		this.setAuthenticationFailureHandler(failureHandler);
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        String state = request.getParameter("params");
        String isLogout = request.getParameter("logout");
        if(!state.equals(null)){
			JSONObject jsons=JSONObject.parseObject(URLDecoder.decode(state,"UTF-8"));
			username = jsons.getString("j_username");
			password = jsons.getString("j_password");
        }else{
        	throw new AuthenticationServiceException("注销用户，返回登录");
        }
        ...
        try {                
            ...
            if (auth == null || !auth.isAuthenticated()) {  //验证失败
            	...
            	//获得当前用户的角色信息   并放入 session中
            	StringBuffer roleStr=new StringBuffer("");
            	for(GrantedAuthority ga :auth.getAuthorities()){
            		roleStr.append(ga.getAuthority());
            		roleStr.append(",");
            	}
            	if(roleStr.length()!=0){
            		session.setAttribute("ROLE_INFO",roleStr.substring(0,roleStr.length()-1));
            	}
            	else{
            		session.setAttribute("ROLE_INFO",roleStr);
            	}
            	//String str=session.getAttribute("ROLE_INFO").toString();
            }  
