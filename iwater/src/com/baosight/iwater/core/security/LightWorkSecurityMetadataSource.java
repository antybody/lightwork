...
public class LightWorkSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
	...
	private void loadAllResourceData() {
		...
		List<List<String>> list=loginService.selRolePower();
		for(int i=0;i<list.size();i++){
			Collection<ConfigAttribute> attsno = new ArrayList<ConfigAttribute>();
			for(int j=0;j<list.get(i).size();j++){
				if(j!=0){
					attsno.add(new SecurityConfig(list.get(i).get(j)));
				}
			}
			attsno.add(new SecurityConfig("ROLE_ADMIN"));  //每个资源默认可以被最高角色访问
			map.put(list.get(i).get(0), attsno);
		}
