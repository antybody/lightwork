...
public class LightWorkUserDetailServiceImpl implements UserDetailsService {	
	...
	protected List<UserDetails> loadUsersByUsername(String username) {
		//查询对应账号的用户
		com.baosight.iwater.system.login.pojo.User user=loginDao.selectByUserCode(username);
	...
	private Set<GrantedAuthority> loadUserAuthorities(String username) {
		...
		List<String> list=loginDao.selRoleByUserCode(username);
		for(int i=0;i<list.size();i++){
			authSet.add(new SimpleGrantedAuthority(list.get(i)));
		}
		//每个用户默认拥有ROLE_USER角色
