...
public  class CacheManager {
    ...
    //private static CacheManager ru = new CacheManager();
    private  ApplicationContextUtil applicationUtil;
    public CacheManager() {
    	while(jedis==null){
    		ApplicationContext context = applicationUtil.getContext();
    		//ApplicationContext context = new ClassPathXmlApplicationContext("classpath:spring-mybatis.xml");  
    		JedisConnectionFactory jedisConnectionFactory = (JedisConnectionFactory) context.getBean("connectionFactory"); 
    ...
    public String get(String key){
        ...
        try {
            ...
            returnResource(jedisConnection, jedis);
        }
        return value;
    }
     ...
     * @param jedis
     */
    public static void returnResource(JedisConnection jedisConnection1 , Jedis jedis) {
        if (jedis != null) {
        	jedisConnection1.close();
        }
    }

//	public static CacheManager getRu() {
//		return ru;
//	}
//
//	public static void setRu(CacheManager ru) {
//		CacheManager.ru = ru;
//	}
	//List<Map<String,String>>中删除某条记录
	//delete 删除的记录对应的state为“2”
	//map1是删除的条件
	public String delListById(String key, Map<String, String> map1) {
		if(map1.size()!=0&&!key.equals("")){
			List<Map<String,String>> str1=null;
			try {
				//得到该key的value集合
				str1 = (List<Map<String, String>>) SerializeUtil.unserialize(jedis.get(key.getBytes("utf-8")));
			...
			if(str1.size()>0){
				...
				for(int i=0;i<str1.size();i++){
					Map<String,String> map11=str1.get(i);
					int state=1;
					for (Map.Entry<String, String> entry1 : map1.entrySet()) {
						if(!entry1.getValue().equals(map11.get(entry1.getKey()))){
							state=0;
						}
					}
					if(state==1){
				...
				try {
					jedis.set(key.getBytes("utf-8"), SerializeUtil.serialize(str1));
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}finally {
					 returnResource(jedisConnection, jedis);
	...
	//update 修改的记录对应的state为“3”,map1存放的是条件，map2存放的是更新的值
	public String updateListById(String key, Map<String, String> map1, Map<String, String> map2) {
		if(map1.size()!=0&&map2.size()!=0&&!key.equals("")){
			List<Map<String,String>> str1=null;
			try {
				//得到该key的value集合
...
//insert 插入的记录对应的state为“1”
	public String insertListById(String key, Map<String, String> map) {
		if(map.size()!=0&&!key.equals("")){
			List<Map<String,String>> str1=null;
			try {
				//得到该key的value集==,合
				str1 = (List<Map<String, String>>) SerializeUtil.unserialize(jedis.get(key.getBytes("utf-8")));
			...
			if(str1.size()>0){
				Map<String, String> map1234=str1.get(0);
				Map<String, String> map123=new HashMap<String, String>();
				map123.putAll(map1234);
				String UUid=UUID.randomUUID().toString().replace("-","");
				for (Map.Entry<String, String> entry : map123.entrySet()) {
					if(!map.containsKey(entry.getKey())){
						map123.put(entry.getKey(), "");
					}
				}
				for (Map.Entry<String, String> entry : map.entrySet()) {
					if(!map123.containsKey(entry.getKey())){
						return"0";
					}else{
						map123.put(entry.getKey(),entry.getValue());
					}
				}
				map123.put("id",UUid);
				map123.put("state","1");
